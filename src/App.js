...
import Form from "./Components/Form";
import swal from "sweetalert";
import { signInWithEmailAndPassword, createUserWithEmailAndPassword, updateProfile, GoogleAuthProvider, getAuth, signInWithPopup, signOut } from "firebase/auth";
import Navbar from "./Components/Navbar";
import { getDatabase, ref, push, onValue, remove, set } from 'firebase/database';
import { auth } from './firebase';
...
const App = () => {
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      setLogin(true);
      setUserName(user.displayName);
      swal("Success", `Welcome, ${user.displayName}! You have successfully logged in with Google.`, "success");
    } catch (error) {
      console.error(error);
      alert("Error : Unable to sign in with Google");
    }
  };
